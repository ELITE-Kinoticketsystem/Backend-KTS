	...
	}{
		{
			...
			setExpectations: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(query).WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg()).WillReturnRows(sqlmock.NewRows([]string{
					...
					AddRow(order.Order.ID, order.Order.Totalprice, order.Order.IsPaid, order.Order.PaymentMethodID, order.Order.UserID, order.Tickets[0].Ticket.ID, order.Tickets[0].Ticket.Validated, order.Tickets[0].Ticket.Price, order.Tickets[0].Ticket.PriceCategoryID, order.Tickets[0].Ticket.OrderID, order.Tickets[0].Ticket.EventSeatID, order.Tickets[0].PriceCategory.ID, order.Tickets[0].PriceCategory.Price, order.Tickets[0].PriceCategory.CategoryName, order.Tickets[0].Seat.ID, order.Tickets[0].Seat.RowNr, order.Tickets[0].Seat.ColumnNr, order.Tickets[0].Seat.SeatCategoryID, order.Tickets[0].Seat.CinemaHallID, order.Tickets[0].Seat.Type, order.Tickets[0].SeatCategory.ID, order.Tickets[0].SeatCategory.CategoryName))
			},
			expectOrder:   &order,
			expectedError: nil,
		},
