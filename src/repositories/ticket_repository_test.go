...
func TestGetTicketById(t *testing.T) {
	...
	query := "SELECT tickets.id AS \"tickets.id\", tickets.validated AS \"tickets.validated\", tickets.price AS \"tickets.price\", seats.id AS \"seats.id\", seats.y AS \"seats.y\", seats.x AS \"seats.x\", seats.row_nr AS \"seats.row_nr\", seats.column_nr AS \"seats.column_nr\", seats.seat_category_id AS \"seats.seat_category_id\", seats.cinema_hall_id AS \"seats.cinema_hall_id\", seats.type AS \"seats.type\", events.id AS \"events.id\", events.title AS \"events.title\", events.start AS \"events.start\", events.end AS \"events.end\", events.description AS \"events.description\", events.event_type AS \"events.event_type\", events.cinema_hall_id AS \"events.cinema_hall_id\", events.is3d AS \"events.is3d\", orders.id AS \"orders.id\", orders.totalprice AS \"orders.totalprice\", orders.is_paid AS \"orders.is_paid\", orders.payment_method_id AS \"orders.payment_method_id\", orders.user_id AS \"orders.user_id\" FROM `KinoTicketSystem`.tickets INNER JOIN `KinoTicketSystem`.event_seats ON (event_seats.id = tickets.event_seat_id) INNER JOIN `KinoTicketSystem`.seats ON (seats.id = event_seats.seat_id) INNER JOIN `KinoTicketSystem`.events ON (events.id = event_seats.event_id) INNER JOIN `KinoTicketSystem`.orders ON (orders.id = tickets.order_id) WHERE tickets.id = ?;"
