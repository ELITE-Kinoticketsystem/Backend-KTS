...
func TestGetMovieById(t *testing.T) {
	...
	query := "SELECT movies.id AS \"movies.id\", movies.title AS \"movies.title\", movies.description AS \"movies.description\", movies.banner_pic_url AS \"movies.banner_pic_url\", movies.cover_pic_url AS \"movies.cover_pic_url\", movies.trailer_url AS \"movies.trailer_url\", movies.rating AS \"movies.rating\", movies.release_date AS \"movies.release_date\", movies.time_in_min AS \"movies.time_in_min\", movies.fsk AS \"movies.fsk\", genres.id AS \"genres.id\", genres.genre_name AS \"genres.genre_name\", actors.id AS \"actors.id\", actors.name AS \"actors.name\", actors.birthdate AS \"actors.birthdate\", actors.description AS \"actors.description\", actor_pictures.pic_url AS \"actor_pictures.pic_url\", reviews.id AS \"reviews.id\", reviews.rating AS \"reviews.rating\", reviews.comment AS \"reviews.comment\", reviews.datetime AS \"reviews.datetime\", reviews.is_spoiler AS \"reviews.is_spoiler\", reviews.user_id AS \"reviews.user_id\", reviews.movie_id AS \"reviews.movie_id\", users.username AS \"users.username\" FROM `KinoTicketSystem`.movies LEFT JOIN `KinoTicketSystem`.movie_genres ON (movie_genres.movie_id = movies.id) LEFT JOIN `KinoTicketSystem`.genres ON (genres.id = movie_genres.genre_id) LEFT JOIN `KinoTicketSystem`.movie_actors ON (movie_actors.movie_id = movies.id) LEFT JOIN `KinoTicketSystem`.actors ON (actors.id = movie_actors.actor_id) LEFT JOIN `KinoTicketSystem`.actor_pictures ON (actor_pictures.actor_id = actors.id) LEFT JOIN `KinoTicketSystem`.reviews ON (reviews.movie_id = movies.id) LEFT JOIN `KinoTicketSystem`.users ON (users.id = reviews.user_id) WHERE movies.id = ?;"
	...
	}{
		{
			...
			setExpectations: func(mock sqlmock.Sqlmock, id *uuid.UUID) {
				mock.ExpectQuery(query).WithArgs(utils.EqUUID(id)).WillReturnRows(
					sqlmock.NewRows([]string{"movies.id", "movies.title", "movies.description", "movies.banner_pic_url", "movies.cover_pic_url", "movies.trailer_url", "movies.rating", "movies.release_date", "movies.time_in_min", "movies.fsk", "genres.id", "genres.genre_name", "actors.id", "actors.name", "actors.birthdate", "actors.description", "reviews.id", "reviews.rating", "reviews.comment", "reviews.datetime", "reviews.is_spoiler", "reviews.user_id", "reviews.movie_id", "users.username", "actor_pictures.pic_url"}),
				)
			},
			expectedMovie: nil,
			expectedError: kts_errors.KTS_NOT_FOUND,
			...
			setExpectations: func(mock sqlmock.Sqlmock, id *uuid.UUID) {
				mock.ExpectQuery(query).WithArgs(utils.EqUUID(id)).WillReturnRows(
					sqlmock.NewRows(
						...
						&smplFullMovie.ID, &smplFullMovie.Title, &smplFullMovie.Description, &smplFullMovie.BannerPicURL, &smplFullMovie.CoverPicURL, &smplFullMovie.TrailerURL, &smplFullMovie.Rating, &smplFullMovie.ReleaseDate, &smplFullMovie.TimeInMin, &smplFullMovie.Fsk, &smplFullMovie.Genres[0].ID, &smplFullMovie.Genres[0].GenreName, &smplFullMovie.Actors[0].ID, &smplFullMovie.Actors[0].Name, &smplFullMovie.Actors[0].Birthdate, &smplFullMovie.Actors[0].Description, &smplFullMovie.Reviews[0].Review.ID, &smplFullMovie.Reviews[0].Review.Rating, &smplFullMovie.Reviews[0].Review.Comment, &smplFullMovie.Reviews[0].Review.Datetime, &smplFullMovie.Reviews[0].Review.IsSpoiler, &smplFullMovie.Reviews[0].Review.UserID, &smplFullMovie.Reviews[0].Review.MovieID, &smplFullMovie.Reviews[0].Username, &smplFullMovie.Actors[0].ActorPictureUrls,
					),
				)
			},
